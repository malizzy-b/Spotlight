...
class SpotlightSupabaseUser extends BaseAuthUser {
  SpotlightSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> spotlightSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = SpotlightSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
